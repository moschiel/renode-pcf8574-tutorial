...
{
    HAL_UART_Transmit(&huart2, (const uint8_t *)text, (uint16_t)strlen(text), 100U);
}
...
{
    uint8_t value;
    if(HAL_I2C_Master_Receive(&hi2c1, PCF_ADDRESS, &value, 1, 100U) != HAL_OK)
    {
        fail("ERROR: PCF8574 read\r\n");
    }
    return value;
}
...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  print_line("STM32F407 / PCF8574 lab\r\n");
  validate_pcf8574();
